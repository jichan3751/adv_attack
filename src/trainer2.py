...
class Trainer(object):
    ...
    def setup(self):
        ...
        fetch_checkpoint_hack(self.sess, self.model.weights_nat) # load naturual model
    ...
    def hack1(self):
        # check loss of first 10 imanges
        img_indices = range(10)

        for img_idx in img_indices:
            x_one, y_one = self.train_dataset.load_index([img_idx])
            fd = {
                self.model.x_pl: x_one,
                self.model.y_pl: y_one
            }

            loss, corr, dd = self.sess.run([self.model.loss,self.model.num_correct, self.model.distance], feed_dict = fd)
            print('img_idx %d loss %f corr %d, dist %f'%(img_idx, loss, corr, dd))
...
class MyModel(object):
    ...
    def setup(self):
        ...
        ## for hack
        self.distance = tf.norm(self.adv_image - self.x_pl)
...
class Dataset(object):
    ...
    def load_index(self, idx):
        return self.x[idx,:].reshape((-1,784)), np.array(self.y[idx]).reshape((1,))
